...
public class TreeRecyclerAdapter extends BaseRecyclerAdapter<TreeItem> {
    ...
    public TreeRecyclerAdapter(Context mContext) {
        super(mContext);
    }
                ...
                public void onClick(View v) {
                    ...
                    if (getCheckItem().checkClick(layoutPosition)) {
                        ...
                        if (type != TreeRecyclerType.SHOW_ALL && item instanceof TreeItemGroup) {
                            //展开,折叠
                            expandOrCollapse(((TreeItemGroup) item), itemPosition);
                        } else {
     ...
     */
    private void expandOrCollapse(TreeItemGroup treeItemGroup, int itemPosition) {
        boolean expand = treeItemGroup.isExpand();
        List<TreeItem> datas = getDatas();
        treeItemGroup.setExpand(!expand);
        if (onExpandListener != null) {
            onExpandListener.onExpand(itemPosition);
        }
        treeItemGroup.notifyExpand();
        for (int i = 0; i < datas.size(); i++) {
            TreeItem data = datas.get(i);
            TreeItemGroup parentItem = data.getParentItem();
            if (parentItem != null) {
                if (treeItemGroup.isExpand()) {
                    if (treeItemGroup != parentItem && parentItem.isExpand()) {
                        parentItem.setExpand(false);
                        parentItem.notifyExpand();
                    }
                }
            }
        }
