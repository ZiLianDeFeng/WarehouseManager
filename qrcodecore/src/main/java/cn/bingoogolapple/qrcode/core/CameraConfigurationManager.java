...
final class CameraConfigurationManager {
    ...
    public void largeZoom(Camera camera) {
        Camera.Parameters parameters = camera.getParameters();
        int zoom = parameters.getZoom();
        Log.e("camera", "largeZoom: " + zoom);
        parameters.setZoom((int)(zoom * 1.2));
        camera.setDisplayOrientation(getDisplayOrientation());
        camera.setParameters(parameters);
    }
...
//        }
        if (parameters.isZoomSupported()) {
            Log.e("setZoom", "max-zoom:" + parameters.getMaxZoom());
            Log.i("0000", "tenDesiredZoom:" + tenDesiredZoom);
            int i = parameters.getMaxZoom() / tenDesiredZoom;
            parameters.setZoom(i);
        } else {
            Log.e("setZoom", "Unsupported zoom.");
        }
    }
}